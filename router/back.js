import React, { useState, useEffect } from '../index.js';
const GLOBAL_CACHE = 'cache-v2';

var window = window || {};
var sessionStorage = sessionStorage || {};
var localStorage = localStorage || {};
var AppLocals = AppLocals || {};

window.ThisStorage = new Proxy(
  { ...sessionStorage },
  {
    get(target, name) {
...
window.fetch = async function (url, options) {
  ...
  if (sessionStorage.MuC_ID) {
    ...
    options.headers['Session-ID'] = sessionStorage.MuC_ID;
  }
  // concat sessionStorage.MuC_ID and url
  var mix = sessionStorage.MuC_ID ? sessionStorage.MuC_ID + url : url;
  // hash mix to make sure they are not tampered with
  options.headers['Hash-ID'] = mix.hashCode();
  let cache_key = mix.hashCode();
...
window._reactKeys = AppLocals['_reactKeys'];
